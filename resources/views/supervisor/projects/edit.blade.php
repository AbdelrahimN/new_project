...
@endsection
@section('container')
    <form action="{{ route('supervisor.projects.update',$record->id) }}" method="POST" class="form-control" id="form">
        @csrf
        <div class="row">
            <div class="col-md-6 col-lg-6" >
                ...
                <textarea name="description" placeholder="Description" class="form-control" autocomplete="off">{{ $record->description }}</textarea>
                @error('description')
                    <h5 class="alert alert-danger">{{ $message }}</h5>
                @enderror
            </div>
